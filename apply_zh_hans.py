...
def localize_package_catalog(root: Path) -> int:
    ...
    descriptions: dict[str, tuple[str, str]] = {
        "PerfHUD": (
            "实时 CPU / GPU / 内存透明悬浮监控",
            "在 SpringBoard 上显示系统 CPU、GPU 和内存实时占用。悬浮窗会按设备物理方向适配横竖屏，不使用背景或文字阴影，并根据每项负载独立变色。GPU 数据不可用时会安全显示 GPU --。刷新间隔可在 0.25～5.00 秒之间调整。",
        ),
        "App Switcher Grid": (
            "将多任务切换器改成网格式布局",
...
def main() -> None:
    ...
    total += patch_literals(root, "Cyanide/SettingsViewController.m", {
        ...
        objc("Transparent CPU / GPU / RAM performance HUD. The overlay explicitly follows physical device orientation across portrait/landscape games and uses no text shadow. CPU keeps the last valid sample through brief sub-second tick gaps instead of flashing --. GPU uses IOKit statistics when available and safely shows -- when unavailable. Refresh interval supports 0.25-5.00 seconds; the HUD pauses while the screen is locked or asleep."): objc("透明的 CPU / GPU / 内存性能悬浮窗。横竖屏会按设备物理方向自动适配，并且不使用文字阴影。亚秒级刷新时如果 CPU tick 短暂没有推进，会继续显示上一帧有效 CPU 数值，避免闪成 --。GPU 会在可用时读取 IOKit 统计数据，不可用时安全显示 --。刷新间隔支持 0.25～5.00 秒；锁屏或休眠时悬浮窗会暂停刷新。"),
